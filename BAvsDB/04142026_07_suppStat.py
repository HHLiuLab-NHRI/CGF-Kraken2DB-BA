...
STAT_DIR = '../statistics/'
OUTPUT_TEXT_FILE = os.path.join(STAT_DIR, 'nFunDB_Metrics.txt')
...
def get_stats(input_file, read_label):
    ...
    out_file = os.path.join(STAT_DIR, f"nFunDB_EffectSizes_{read_label}.csv")
    stats_df.to_csv(out_file, index=False)
    log_and_print(f"\nWrote comprehensive effect size/prevalence table to -> {out_file}")

# Run for both R1 and R2
try: get_stats('../k2GenusSummary_R1_nFunDB/genusAverage.csv', 'R1')
except Exception as e: log_and_print(f"Could not process R1: {e}")

try: get_stats('../k2GenusSummary_R2_nFunDB/genusAverage.csv', 'R2')
